"""	Lab03ejercicio1.py
	...
	s: float // ENTRADA: Almacena el valor de la sucesion.
	r: float // ENTRADA: Almacena el valor del radio.
	ar: float // ENTRADA: Almacena el valor del area.
	n: int // ENTRADA: Almacena el valor del numero natural.
	su: int // ENTRADA: Almacena el valor de la suma.
...
if o=="n":
	...
	while ( 0<=i<=2 ):
		...
		if (p == 1):
			...
			assert(n>0)
			su=sum(j**2 for j in (1,n+1))
			assert(su==sum(j**2 for j in (1,n+1)))
...
# Postcondicion: 
assert( o != "s" or s==l*a or su==sum(j**2 for j in (1,n+1) or ar==(3.1416*(r**2))))
